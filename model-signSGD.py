    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                if p.grad is None:
                    continue
                # Update based on the sign of the gradient
                grad = p.grad.data
                sign_grad = grad.sign()
                p.data.add_(-group['lr'] * sign_grad)
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, use_alternate_attention=False):
        device = idx.device
        b, t = idx.size()
