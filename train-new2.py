...
def get_full_batch(split):
    ...
    if split == 'train':
        ...
        data = np.memmap(os.path.join(data_dir, 'val.bin'), dtype=np.uint16, mode='r')
    ix = torch.randint(len(data) - block_size, (batch_size * gradient_accumulation_steps,))
    x = torch.stack([torch.from_numpy((data[i:i+block_size]).astype(np.int64)) for i in ix])
    y = torch.stack([torch.from_numpy((data[i+1:i+1+block_size]).astype(np.int64)) for i in ix])
    if device_type == 'cuda':
        # pin arrays x,y, which allows us to move them to GPU asynchronously (non_blocking=True)
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process:
        X_full, Y_full = get_full_batch('train')
        gradients = []
        for i in range(X_full.size(0)):
            x_sample = X_full[i:i+1]
            y_sample = Y_full[i:i+1]
            optimizer.zero_grad()
