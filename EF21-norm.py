  ...
  def run(self):
    ...
    while True:
        ...
        if wcfg.cmd == "bcast_g_c0":
            ...
            k = 0
            gradient_norm = torch.norm(torch.cat([g.view(-1) for g in wcfg.input_for_cmd[0]])).item()
            for p in model.parameters(): 
                p.data = p.data - ncfg.gamma / gradient_norm * wcfg.input_for_cmd[0][k]
                k = k + 1
...
def main():
    ...
    print(f"Start training {nn_config.model_name}@{nn_config.dataset} for K={nn_config.KMax} iteration. EF21-norm", available_devices)
