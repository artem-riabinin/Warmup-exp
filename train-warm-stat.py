...
def get_batch_small(split):
    ...
    if split == 'train':
        ...
        data = np.memmap(os.path.join(data_dir, 'val.bin'), dtype=np.uint16, mode='r')
    ix = torch.randint(len(data) - block_size, (8,))
    x = torch.stack([torch.from_numpy((data[i:i+block_size]).astype(np.int64)) for i in ix])
    y = torch.stack([torch.from_numpy((data[i+1:i+1+block_size]).astype(np.int64)) for i in ix])
    if device_type == 'cuda':
        # pin arrays x,y, which allows us to move them to GPU asynchronously (non_blocking=True)
...
def estimate_loss():
    ...
    return out
#####
def calculate_sharpness(model, gradients, iter_num, vs, m_iter: int = 30, tol: float = 1e-4):
    ...
    eigs, eigvs = linalg.lobpcg(hvp, vs, maxiter=m_iter, tol=tol)
    return eigs, eigvs
#####
...
#####
    if ((iter_num % eval_interval == 0 and iter_num <= 4000) or (iter_num % eval_interval_2 == 0 and iter_num > 4000)) and master_process:
        ...
        logits, loss = model(X, Y, use_alternate_attention=True)
        ...
        eigs, vs = calculate_sharpness(model, gradients_for_hess, iter_num, vs)
